...
_CONFIG_PATTERNS = [
    re.compile(
        r"\b(change|set|update|configure|adjust|toggle|switch)\b.*?"
        r"\b(setting|config|sensitivity|voice|volume|model|theme|provider|rate|vad)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(turn\s+on|turn\s+off|enable|disable)\s+(developer\s+mode|wake\s+word|vad|audio|auto\s+memory)\b",
        re.IGNORECASE,
    ),
]
...
_MCP_PATTERNS = [
    re.compile(
        r"\b(connect|add|register|configure|install)\s+(the\s+)?(mcp|model\s+context\s+protocol|mcp\s+server)\b",
        re.IGNORECASE,
    ),
...
_ARCH_LARGE_PATTERNS = [
    re.compile(
        r"\b(replace|rewrite|redesign|overhaul)\s+(your\s+)?(entire\s+)?"
        r"(orchestration|architecture|memory\s+subsystem|event\s+bus|runtime\s+engine)\b",
        re.IGNORECASE,
    ),
...
_CODE_SMALL_PATTERNS = [
    re.compile(
        r"\b(add|implement|write|create|code)\s+(a\s+)?(small\s+)?"
        r"(function|helper|tool|capability|module|calculator|formatter)\b",
        re.IGNORECASE,
    ),
