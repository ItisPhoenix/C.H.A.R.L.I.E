...
class Suggestion:
    ...
    actions: List[
        Dict
    ]  # Available actions e.g. [{"label": "Yes", "action": "do_this"}, {"label": "No", "action": "dismiss"}]
...
class SuggestionEngine:
        ...
        try:
            ...
            for event in schedule:
                ...
                if now <= start <= window:
                    ...
                    existing = [s for s in self._suggestions if s.type == "meeting_reminder" and event_id in s.id]
                    if existing:
                        continue
    ...
    def _check_idle_resume(self):
        ...
        suggestion = Suggestion(
            ...
            actions=[{"label": "Yes", "action": "resume"}, {"label": "No", "action": "dismiss"}],
            priority=3,
    ...
    def _check_pattern_automation(self):
        ...
        for pattern in patterns:
            ...
            existing = [s for s in self._suggestions if s.type == "automate" and desc in s.message]
            if existing:
                continue
            ...
            suggestion = Suggestion(
                ...
                actions=[{"label": "Yes, automate", "action": "automate"}, {"label": "Not now", "action": "dismiss"}],
                priority=5,
        ...
        for task in pending_tasks:
            ...
            try:
                ...
                if 0 < hours_until <= self.TRIGGERS["deadline_alert"]["hours_before"]:
                    # Check if we already suggested this
                    existing = [s for s in self._suggestions if s.type == "deadline" and task["id"] in s.message]
                    if existing:
                        continue
