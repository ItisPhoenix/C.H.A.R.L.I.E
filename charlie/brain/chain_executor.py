...
class ChainContext:
    ...
    def to_llm_context(self, brain_ref: Any = None) -> str:
        ...
        if is_remote:
            ...
            active_win = (
                getattr(brain_ref, "active_window", "Unknown")
                if brain_ref
                else "Unknown"
            )
...
class ChainExecutor:
    ...
    def add_step(
        self, tool_name: str, args: Dict[str, Any], output: str, tier: RiskTier
    ):
        ctx = self.get_active_context()
        if ctx:
            is_error = isinstance(output, str) and (
                output.startswith("Error") or "error" in output[:100].lower()
            )
            step = StepResult(
    ...
    def is_chain_complete(self) -> bool:
        ...
        if ctx.steps and any(
            "final_answer" in str(s.output).lower() for s in ctx.steps[-2:]
        ):
            return True
    ...
    def _prune_current_history(
        self, history: list, keep_last: int = 10, trigger_at: int = 15
    ) -> list:
        """Prune history when it grows too large during chain execution.

        Keeps only the last *keep_last* non-system messages once the list
        exceeds *trigger_at* messages.  System messages at the front are
        preserved so the persona survives the trim.
        """
        if len(history) <= trigger_at:
            return history
        ...
        if dropped > 0:
            logger.debug(
                f"history_pruned | dropped={dropped} msgs | "
                f"kept={len(pruned)}/{len(history)}"
            )
        return pruned
    ...
    async def execute_chain(
        ...
        try:
            while not self.is_chain_complete():
                    logger.debug(
                        f"chain_loop_start | step={chain.current_step_count + 1}"
                    )
                    ...
                    status_label = (
                        "THINKING..."
                        if current_depth == 0
                        else f"PLANNING STEP {current_depth}"
                    )
                    ...
                    cleaned_history = self._prune_history_smart(
                        [
                            ...
                            or "Current Time:" not in m.get("content", ""))
                        ]
                    )
                    ...
                    try:
                            ...
                            **{k: v for k, v in payload.items() if k not in ("messages", "model", "temperature")},
                            ...
                            if content:
                                ...
                                if (
                                    ...
                                    fa_start = current_full.find(fa_marker) + len(
                                        fa_marker
                                    )
                        ...
                        if not full_response.strip():
                            logger.warning(
                                f"chain_empty_response | step={current_depth}"
                            )
                    ...
                    if (
                        ...
                        and chain.current_step_count < chain.max_steps
                    ):
                        ...
                        if retry_count < 3:
                            ...
                            for step in reversed(chain.steps):
                                if hasattr(step, "output") and step.output:
                                    last_obs = self._compress_tool_output(
                                        str(step.output)
                                    )
                                    break
                            ...
                            logger.error(
                                "neural_link_failure | max_retries_exceeded | aborting_chain"
                            )
                            self._recovery_retries = 0
                    ...
                    if not final_answer and action == "none":
                        ...
                        if final_answer and final_answer != full_response.strip():
                            logger.debug(
                                f"chain_json_leak_recovered | cleaned_len={len(final_answer)}"
                            )
                    ...
                    if action == "none" and final_answer:
                        ...
                        if is_phantom and chain.current_step_count < chain.max_steps:
                            ...
                            logger.warning(
                                f"phantom_processing_detected | injecting_followup | phrase={final_answer[:60]}"
                            )
                        ...
                        for s in fragments:
                            ...
                            s_clean = s_clean.replace("<endofturn>", "").replace(
                                "<startofturn>", ""
                            )
                            ...
                            if (
                                any(m in s_clean.lower() for m in thought_markers)
                                and len(s_clean) < 100
                            ):
                                continue
                    ...
                    if action not in ("none", ""):
                        ...
                        if chain.source == "local" and action != "none":
                            import random
                            bridges = ["Checking that, Sir.", "Looking into it.", "One moment, Sir.", "Processing data.", "Gathering info."]
                        ...
                        try:
                            ...
                            if obs == CONFIRMATION_PENDING:
                                ...
                                tier = (
                                    get_tool_tier(tool_func)
                                    if tool_func
                                    else RiskTier.TIER_1
                                )
                                ...
                                if tier == RiskTier.TIER_2:
                                    ...
                                    timeout_seconds = (
                                        120  # longer for destructive actions
                                    )
                            ...
                            current_history.append(
                                {"role": "system", "content": f"OBSERVATION: {obs}"}
                            )

                            # Prevent history from growing unbounded during long chains
                            current_history = self._prune_current_history(current_history)

                            if action in ("shutdown", "system_reboot", "clarification", "standby"):
                                if str(obs).startswith(("Error", "Failed")):
                                    final_response_text = f"I'm sorry, Sir, but I failed to execute the `{action}` tool. {obs}"
                                elif action in ("clarification", "shutdown", "standby"):
                                    final_response_text = ""
                                else:
                                    final_response_text = (
                                        final_answer or f"Executed {action}."
                                    )
                                break
                            ...
                            self.add_step(
                                action, action_input, f"Error: {e}", RiskTier.TIER_0
                            )
                            continue
                            ...
                            for fr_s in fr_fragments:
                                ...
                                if fr_s_clean and len(fr_s_clean) > 5:
                                    ...
                                    if norm_fr not in sent_sentences_global:
                                        ...
                                        speech_chunks = [fr_s_clean] if len(fr_s_clean) <= 150 else re.split(r"(?<=[,;])\s+", fr_s_clean)
                                        for chunk in speech_chunks:
                                            if len(chunk) > 5:
                        ...
                        if final_response_text and brain:
                            brain._safe_put(brain.status_q, {
                                "type": "CHAT_MSG",
                                "speaker": "CHARLIE",
                        ...
                        if is_json or is_shorthand:
                            logger.debug(
                                f"speech_suppressed | raw_leak_detected | {raw_res[:30]}..."
                            )
                            final_response_text = (
                                "Task complete, Sir." if action != "none" else ""
                            )
            ...
            logger.exception(
                f"query_cycle_failed | error_type={type(e).__name__} | msg={e}"
            )
            if "brain" in locals():
                brain._safe_put(
                    brain.tts_q,
