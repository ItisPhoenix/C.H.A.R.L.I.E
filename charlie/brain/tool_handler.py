...
class ToolHandler:
                ...
                if not skip_guardian:
                    ...
                    if allowed == CONFIRMATION_PENDING:
                        ...
                        if source == "local" or source == "all":
                            ...
                            self.brain._safe_put(self.brain.tts_q, {"type": "SPEAK", "content": reason})
    ...
    def _tool_move_widget(self, args: dict[str, Any]) -> str:
        ...
        if self.brain.status_q:
            self.brain._safe_put(
                self.brain.status_q, {"type": "MOVE_WIDGET", "content": {"name": name, "target": target, "zone": zone}}
            )
    ...
    def _tool_cast_media(self, args: dict[str, Any]) -> str:
        ...
        if not query and not url:
            return "Error: No media source provided."
        if query:
            return self._tool_open_app({"name": "youtube", "query": query})

        # Guard against os.startfile being abused to download-and-execute
        # arbitrary executables via HTTP redirects or local file paths.
        ...
        if parsed.scheme and parsed.scheme not in ("http", "https", ""):
            logger.warning("dangerous_scheme_blocked | target=%s", url_str)
            return "Access denied: only http(s) and local file targets are allowed, Sir."

        if check_dangerous_file_type(url_str):
            logger.warning("dangerous_file_type_blocked | target=%s", url_str)
            return "Access denied: target has a dangerous file extension, Sir."

        # Path Traversal Guard for local file targets (no scheme → local)
        if not parsed.scheme and not self._is_safe_path(url_str):
            logger.warning("unauthorized_media_cast_blocked | target=%s", url_str)
    ...
    def _tool_get_foreground_app(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            return "Focused window metadata is currently unreadable."
    ...
    def _tool_get_pc_clipboard(self, args: dict[str, Any]) -> str:
        try:
            ...
            return f"Clipboard fetch failed: {e}"
    ...
    def _tool_analyze_screen(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            if loop and loop.is_running():
                future = asyncio.run_coroutine_threadsafe(self.brain.vision_handler.ask_vision(query), loop)
    ...
    def _tool_describe_image(self, args: dict[str, Any]) -> str:
        ...
        try:
            loop = self.brain.loop
            if loop and loop.is_running():
                future = asyncio.run_coroutine_threadsafe(self.brain.vision_handler.analyze_image(path, query), loop)
                return future.result(timeout=VISION_TOOL_TIMEOUT)
            else:
    ...
    def _tool_timer(self, args: dict[str, Any]) -> str:
        ...
        def cb():
            if self.brain.tts_q:
                self.brain._safe_put(
                    self.brain.tts_q, {"type": "SPEAK", "content": f"Sir, your timer for {label} has expired."}
                )
    ...
    def _tool_calculate(self, args: dict[str, Any]) -> str:
        ...
        def _safe_eval(node):
            ...
            if isinstance(node, ast.Num):
                return node.n  # Python < 3.12 compat
    ...
    def _tool_remember_preference(self, args: dict[str, Any]) -> str:
        ...
        if not pref:
            return "Preference text required."
    ...
    def _tool_self_modify(self, args: dict[str, Any]) -> str:
        ...
        if not path or not content:
            return "File and content required."
    ...
    def _tool_apply_edit(self, args: dict[str, Any]) -> str:
        ...
        for r in settings.security.restricted_paths:
            if p_obj.is_relative_to(Path(r).resolve()):
    ...
    def _tool_shutdown(self, args: dict[str, Any]) -> str:
        ...
        self.brain._safe_put(
            ...
            {
                "type": "SPEAK",
                "content": "Acknowledged, Sir. Powering down systems. I'll be standing by on the mobile frequency.",
            },
        )
            ...
            if label:
                for tid, tdata in self.brain.active_timers.items():
                    if tdata["label"].lower() == label.lower():
                        target_id = tid
                        break
            elif self.brain.active_timers:
                target_id = list(self.brain.active_timers.keys())[-1]
            if target_id:
    ...
    def _tool_start_stopwatch(self, args: dict[str, Any]) -> str:
        ...
        with self.brain.timers_lock:
            self.brain.active_stopwatches[name] = time.time()
        self.brain._safe_put(self.brain.status_q, {"type": "WIDGET_SHOW", "content": "time"})
        self.brain._emit_time_update()
        ...
        try:
            ...
            for fmt in ("%I:%M %p", "%H:%M", "%I %p"):
                try:
                    ...
                    continue
            if not target_time:
                return "Format unrecognized. Use 'HH:MM AM/PM'."
    ...
    def _tool_system_reboot(self, args: dict[str, Any]) -> str:
        ...
        self.brain._safe_put(
            ...
            {
                "type": "SPEAK",
                "content": "Initiating system reload protocol. Flushing memory buffers. Reinitializing neural core. Coming back online momentarily, Sir.",
            },
        )
    ...
    def _tool_agent_status(self, args: dict[str, Any]) -> str:
        ...
        registry = self.brain.orchestrator.registry
    ...
    def _tool_deps_analyze(self, args: dict[str, Any]) -> str:
        # analyze_dependencies is a Pattern A @tool function in
        # charlie.tools.research_tools — call it directly.
        from charlie.tools.research_tools import analyze_dependencies
        return analyze_dependencies(path=args.get("path", "."))
