...
def _youtube_result_links(page) -> List[str]:
    """Read visible normal-video result titles from the current YouTube results page."""
...
def _youtube_visible_candidates(page, query: str) -> list[dict]:
    """Group visible result title and duration anchors belonging to one video."""
    ...
    for index in range(min(links.count(), 120)):
        ...
        try:
            ...
            card_text = link.evaluate(
                """node => {
                    const card = node.closest('ytd-video-renderer, ytd-rich-item-renderer');
        ...
        duration = _duration_text_to_seconds(text.split("\n", 1)[0])
...
def youtube_play(query: str) -> Optional[BrowserResult]:
    ...
    def run(page):
        actions.navigate(
            page, f"https://www.youtube.com/results?search_query={quote(query)}", wait_selector="a#video-title"
        )
        marks = _observe(page)
        candidates = [
...
# Flipkart's product cards and filters are rendered from the current page, so keep this
# recipe semantic: inspect visible card text, use rendered labels/inputs, and verify the
# resulting products after every state change. This is intentionally separate from the
# YouTube recipes so a site-layout change here cannot alter the existing media path.
_FLIPKART_STATE: dict[str, object] = {}
_FLIPKART_PRICE_RE = re.compile(r"₹\s*([\d,]+)")
_FLIPKART_RAM_RE = re.compile(
    r"(?:RAM|SYSTEM\s+MEMORY|MEMORY)\s*[:\-]?\s*(\d{1,3})\s*GB\b|"
    r"(?:RAM\s*[:\-]?\s*)?(\d{1,3})\s*GB(?:\s+(?:DDR\d*|LPDDR\d*X?))?\s*RAM\b|"
...
def _flipkart_result(url: Optional[str], answer: str, verification: str, success: bool = False) -> BrowserResult:
    return BrowserResult(
        ...
        verification=verification,
        site="flipkart",
        query=_FLIPKART_STATE.get("query") if isinstance(_FLIPKART_STATE.get("query"), str) else None,
    )
...
def _flipkart_link_has_product_evidence(page, link) -> bool:
    ...
    try:
        ...
        if not href or parsed.scheme not in {"", "http", "https"} or absolute == page.url:
            return False
        evidence = _flipkart_link_evidence(link)
        return len(evidence) >= 20 and _flipkart_price(evidence) is not None
    except Exception:
        return False
...
def _flipkart_price(text: str) -> Optional[int]:
    match = _FLIPKART_PRICE_RE.search(text.replace("\xa0", " "))
    if not match:
        return None
    try:
...
def _flipkart_ram(text: str) -> Optional[int]:
    match = _FLIPKART_RAM_RE.search(text)
    if not match:
        return None
...
def _flipkart_visible_products(page, limit: int = 40) -> list[dict]:
    ...
    seen = set()
    links = page.get_by_role("link")
    for index in range(min(links.count(), limit * 3)):
        link = links.nth(index)
        try:
            if not link.is_visible():
                continue
            href = link.get_attribute("href") or ""
            if not _flipkart_link_has_product_evidence(page, link):
                continue
            absolute = urljoin(page.url, href)
            if absolute in seen:
                continue
            evidence = _flipkart_link_evidence(link)
            price = _flipkart_price(evidence)
            if price is None:
                continue
            title = " ".join(link.inner_text(timeout=700).split())
            if not title:
                title = evidence
            products.append(
                {
                    "title": title[:240],
                    "ram_gb": _flipkart_ram(evidence),
                    "price": price,
                    "url": absolute,
                    "text": evidence,
                }
...
def flipkart_search(query: str) -> BrowserResult:
    ...
    def run(page):
        _FLIPKART_STATE.clear()
        _FLIPKART_STATE.update({"query": query, "ram_gb": None, "max_price": None, "sort": None})
        actions.navigate(page, resolve_site("flipkart") or "https://www.flipkart.com")
        ...
        if search_box is None:
            ...
            actions.navigate(
                page, f"{(resolve_site('flipkart') or 'https://www.flipkart.com').rstrip('/')}/search?q={quote(query)}"
            )
        ...
        _FLIPKART_STATE["results_url"] = page.url
        return _flipkart_result(
            page.url,
...
def _flipkart_filter_ram(page, ram_gb: int) -> BrowserResult:
    _flipkart_dismiss_overlays(page)
    prior_ram = _FLIPKART_STATE.get("ram_gb")
    max_price = _FLIPKART_STATE.get("max_price") if isinstance(_FLIPKART_STATE.get("max_price"), int) else None
    ...
    for attempt in range(3):
        ...
        try:
            ...
            break
    if selected is None:
        _FLIPKART_STATE["ram_gb"] = prior_ram
        _FLIPKART_STATE["filter_failure"] = "ram"
        return _flipkart_result(
            page.url,
    ...
    _FLIPKART_STATE["ram_gb"] = ram_gb
    _FLIPKART_STATE.pop("filter_failure", None)
    _FLIPKART_STATE["results_url"] = page.url
    return _flipkart_result(
        page.url,
...
def _flipkart_filter_price(page, max_price: int) -> BrowserResult:
    _flipkart_dismiss_overlays(page)
    prior_max_price = _FLIPKART_STATE.get("max_price")
    before_url = page.url
    price_control = None
    target_label = None
    ...
    ram_gb = _FLIPKART_STATE.get("ram_gb") if isinstance(_FLIPKART_STATE.get("ram_gb"), int) else None
    _FLIPKART_STATE["max_price"] = max_price
    ...
    if not verified and ram_gb is not None:
        ...
        if ram_result.success:
            ...
            verified, detail = _flipkart_verify_products(products, ram_gb=ram_gb, max_price=max_price)
    if not verified:
        _FLIPKART_STATE["max_price"] = prior_max_price
        _FLIPKART_STATE["filter_failure"] = "price"
...
def _flipkart_sort_low_to_high(page) -> BrowserResult:
    _flipkart_dismiss_overlays(page)
    if _FLIPKART_STATE.get("filter_failure"):
        return _flipkart_result(
            page.url,
    ...
    if not verified:
        ...
        filtered_url = _FLIPKART_STATE.get("results_url")
    ...
    _FLIPKART_STATE["sort"] = "price_low_to_high"
    _FLIPKART_STATE["results_url"] = page.url
    return _flipkart_result(
        page.url,
...
def _flipkart_open_cheapest(page) -> BrowserResult:
    ...
    _FLIPKART_STATE["product_url"] = page.url
    _FLIPKART_STATE["product_title"] = facts["title"]
    return _flipkart_result(
        page.url,
...
def flipkart_action(task: str) -> Optional[BrowserResult]:
    ...
    def run(page):
        ...
        if "go back" in lowered and ("filtered" in lowered or "results" in lowered):
            results_url = _FLIPKART_STATE.get("results_url")
...
def site_search(site_url: str, query: str, site_name: Optional[str] = None) -> Optional[BrowserResult]:
    ...
    def run(page):
        ...
        if site_name and site_name.lower() == "youtube":
            try:
                page.wait_for_url("**/results?search_query=**", timeout=8000)
                page.wait_for_selector('[role="main"]', timeout=5000)
                page.wait_for_selector('a[href*="/watch?v="]', timeout=8000)
                page.wait_for_selector('[role="tab"], yt-chip-cloud-chip-renderer', timeout=5000)
                settled = True
            except Exception:
                settled = False
            if not settled:
                ...
                try:
                    page.wait_for_url("**/results?search_query=**", timeout=5000)
                    page.wait_for_selector('a[href*="/watch?v="]', timeout=5000)
                    settled = True
                except Exception:
                    settled = False
        else:
        ...
        if site_name and site_name.lower() == "youtube":
            ...
            parsed_query = (
                unquote_plus(page.url.split("search_query=", 1)[-1].split("&", 1)[0])
                if "search_query=" in page.url
                else ""
            )
...
def _github_tree_file(page, owner: str, repo: str, query: str) -> Optional[tuple[str, str]]:
    """Find a filename match by browsing repository tree pages, without API/search calls."""
    target = _github_normalize_path(query)
    pending = [(f"https://github.com/{quote(owner)}/{quote(repo)}/tree/main", 0)]
    seen = set()
    while pending and len(seen) < 10:
        ...
        for href, name in _github_links(page):
            ...
            if depth < 2 and f"/{owner}/{repo}/tree/" in href and href not in seen:
                relative = href.split(f"/{owner}/{repo}/tree/main/", 1)[-1]
...
def current_repository_search(task: str, current_url: Optional[str]) -> Optional[BrowserResult]:
    ...
    def run(page):
        ...
        search_url = f"https://github.com/{quote(owner)}/{quote(repo)}/find/main?pattern={quote(query)}"
        ...
        if not candidates:
            tree_match = _github_tree_file(page, owner, repo, query)
            if tree_match:
