...
export function UnavailableWidget({ widget, definition }: { widget: WidgetInstance; definition?: WidgetDefinition | null }): ReactElement {
  return (
    <div className="flex h-full flex-col justify-center text-left" data-widget-unavailable="true">
      ...
      <p className="text-xs leading-relaxed text-amber-100/85">
        {definition?.renderer
          ? `Semantic renderer ${definition.renderer} is not implemented for `
          : "No renderer registered for "}
        <span className="font-mono">{widget.widgetType}</span>.
      </p>
    </div>
  );
}
