...
def test_detect_open_app_defers_website_with_leftover():
    from charlie import core

    result = core._detect_open_app("open youtube and search for cats")
...
def test_detect_open_app_bare_website_still_launches(monkeypatch):
    from charlie import core
    ...
    result = core._detect_open_app("open youtube")
    assert result is not None
    msg, leftover = result
...
# --- core.py: deterministic "<verb> ... on <site>" browser-task fast-path -----

def test_detect_browser_task_intent_matches_play_on_site():
    from charlie import core

    assert core._detect_browser_task_intent("play DL91 on youtube") == "play DL91 on youtube"
    assert core._detect_browser_task_intent("Play DL91 on YouTube") == "Play DL91 on YouTube"


def test_detect_browser_task_intent_matches_search_on_site():
    from charlie import core

    assert core._detect_browser_task_intent("search mechanical keyboards on amazon") is not None


def test_detect_browser_task_intent_ignores_unknown_site():
    from charlie import core

    assert core._detect_browser_task_intent("search for cats on my desktop") is None


def test_detect_browser_task_intent_ignores_non_matching_verb():
    from charlie import core

    assert core._detect_browser_task_intent("what is the weather on my phone") is None
